...
function toObservable(object) {
    const observable = isPromise(object) ? fromPromise_1(object) : object;
...
function addClasses(oldClasses, newClasses) {
    const badType = i => !isSet(i) && !isArray$2(i) && !isString(i);
    ...
    const toSet = i => isSet(i) ? i : isArray$2(i) ? new Set(i) : new Set(i.split(' '));
...
class JsonValidators {
    ...
    static type(requiredType) {
        ...
        return (control, invert = false) => {
            ...
            const isValid = isArray$2(requiredType) ?
    ...
    static minItems(minimumItems) {
        ...
        return (control, invert = false) => {
            ...
            let currentItems = isArray$2(control.value) ? control.value.length : 0;
                            ...
                            for (const subKey of Object.keys(schemaValue)) {
                                if (!hasOwn(combinedObject, subKey) ||
                                    ...
                                    const combinedDependency = isArray$2(combinedObject[subKey]) ?
                                        ...
                                        combinedObject[subKey];
                                    const schemaDependency = isArray$2(schemaValue[subKey]) ?
...
function getTitleMapFromOneOf(schema = {}, flatList = null, validateOnly = false) {
    ...
    if (isArray$2(oneOf) && oneOf.every(item => item.title)) {
        if (oneOf.every(item => isArray$2(item.enum) && item.enum.length === 1)) {
            if (validateOnly) {
                return true;
            }
    ...
    JsonPointer.forEachDeep(compiledSchema, (subSchema, subSchemaPointer) => {
        ...
        if (subSchema.type === 'array' &&
            ...
            if (!arrayMap.has(dataPointer)) {
                const tupleItems = isArray$2(subSchema.items) ? subSchema.items.length : 0;
...
function setRequiredFields(schema, formControlTemplate) {
    ...
    if (hasOwn(schema, 'required') && !isEmpty(schema.required)) {
        fieldsRequired = true;
        let requiredArray = isArray$2(schema.required) ? schema.required : [schema.required];
...
function formatFormData(formData, dataMap, recursiveRefMap, arrayMap, returnEmptyFields = false, fixErrors = false) {
    ...
    let formattedData = isArray$2(formData) ? [] : {};
        ...
        if (hasOwn(newNode, 'dataPointer')) {
            ...
            if (newNode.dataType === 'array' &&
                ...
                if (!newNode.recursiveReference || newNode.options.required) {
                    const arrayLength = Math.min(Math.max(newNode.options.tupleItems + newNode.options.listItems, isArray$2(nodeValue) ? nodeValue.length : 0), newNode.options.maxItems);
...
class JsonSchemaFormService {
    ...
    setArrayItemTitle(parentCtx = {}, childNode = null, index = null) {
        ...
        const isArrayItem = (parentNode.type || '').slice(-5) === 'array' && isArray$2(parentValues);
        ...
        const childValue = isArray$2(parentValues) && index < parentValues.length ?
